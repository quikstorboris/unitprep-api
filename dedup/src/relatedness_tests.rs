...
fn shared_email_across_different_names_is_surfaced() {
    ...
    let email_candidates: Vec<_> = candidates
        .iter()
        .filter(|c| c.signal == RelatednessSignal::SharedEmail)
        .collect();
...
fn shared_alternate_contact_name_across_different_tenants_is_surfaced() {
    ...
    let alt_candidates: Vec<_> = candidates
        .iter()
        .filter(|c| c.signal == RelatednessSignal::SharedAlternateContact)
        .collect();
...
fn shared_home_address_across_different_names_is_surfaced() {
    ...
    let address_candidates: Vec<_> = candidates
        .iter()
        .filter(|c| c.signal == RelatednessSignal::SharedHomeAddress)
        .collect();

...
/// [`apply_underline_edits`]'s doc comment for the splitting rules this
/// implements.
fn compute_underline_replacements(
    document_xml: &str,
    doc: &FlatDocument,
    edits: &[UnderlineEdit],
) -> Result<Vec<(usize, usize, String)>, EditError> {
...
/// Shared by [`compute_underline_replacements`] and [`apply_all_edits`]
/// -- the only difference between "a run touched by underline edits
/// only" and "a run touched by a mix of plain and underline edits" is
/// which fragments it received, not how they get turned into XML.
...
fn build_run_xml(document_xml: &str, run: &RunSpan, text: &str, style: RunStyle) -> String {

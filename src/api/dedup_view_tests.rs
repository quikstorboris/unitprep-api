...
use unitprep_dedup::types::{
    FieldCategory, FieldMismatch, FieldValueMismatch, FlaggedGroup, TenantGroup,
...
fn second_flagged_group_cell_refs_account_for_the_blank_separator_row() {
    ...
    let group_b = TenantGroup {
        key: "jones".to_string(),
        records: vec![
            record("301", "Ann", "Jones", "b@example.com"),
            record("302", "Ann", "Jones", ""),
        ],
    };
    ...
    let report = DedupReport {
        flagged_groups: vec![
            FlaggedGroup {
                group: group_a,
                mismatches: vec![mismatch(vec!["a@example.com", "(blank)"])],
                note: String::new(),
            },
            FlaggedGroup {
                group: group_b,
                mismatches: vec![mismatch(vec!["b@example.com", "(blank)"])],
                note: String::new(),
            },
        ],
        ..Default::default()
    ...
    assert_eq!(
        view.flagged_groups[0].bullets[0].cell_refs,
        vec!["J2", "J3"]
    );
    assert_eq!(
        view.flagged_groups[1].bullets[0].cell_refs,
        vec!["J5", "J6"]
    );
}

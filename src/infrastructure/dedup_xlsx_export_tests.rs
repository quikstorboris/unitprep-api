...
fn generate_xlsx_writes_flagged_group_rows_and_notes() {
    ...
    assert!(
        rows[1][2].contains("AlternateContactPhoneNumber"),
        "note should still carry cell references"
    );
    assert_eq!(rows[2][1], "102");
    assert_eq!(
        rows[2][2], "",
        "note is only written on the group's first row"
    );
}
...
fn generate_xlsx_writes_a_related_tenants_section_without_crashing() {
    ...
    let marker_row = rows
        .iter()
        .find(|row| row[0].contains("Possible related tenants"));

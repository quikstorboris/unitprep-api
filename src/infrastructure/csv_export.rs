...
pub fn generate_outputs(analysis: &AnalysisResults, advisory: bool) -> Result<Vec<ExportFile>> {
    let mut files = Vec::new();

    files.push(generate_net_new_groups_csv(analysis)?);

    files.push(generate_facility_assignments_csv(analysis)?);

    files.push(generate_facility_group_lookup_csv(analysis)?);

    if advisory {
        files.push(generate_advisory_csv(&analysis.batch_run.advisory_issues)?);

        files.push(generate_advisory_json(&analysis.batch_run.advisory_issues)?);
    }

    files.push(generate_batch_json(&analysis.batch_run)?);
...
/// into a deliverable.
pub fn build_zip(files: Vec<ExportFile>) -> Result<Vec<u8>> {
...

    {
        ...
        for file in files {
            zip.start_file(&file.file_name, options).map_err(|err| {
                anyhow::anyhow!("Failed adding '{}' to ZIP: {}", file.file_name, err)
            })?;

            zip.write_all(&file.bytes).map_err(|err| {
                anyhow::anyhow!("Failed writing ZIP entry '{}': {}", file.file_name, err)
            })?;
        }

        zip.finish()
            .map_err(|err| anyhow::anyhow!("Failed finalizing ZIP: {}", err))?;
    }
...
fn generate_batch_json(batch: &unitprep_unit_group::BatchRun) -> Result<ExportFile> {
    Ok(ExportFile {
        file_name: "batch_run.json".to_string(),
        bytes: serde_json::to_vec_pretty(batch)?,
    })
}

fn infer_output_name(analysis: &AnalysisResults) -> String {
    if let Some(first_facility) = analysis.batch_run.facilities.first() {
        return format!(
            "{}_net_new_unit_groups.csv",
            sanitize_name(&first_facility.name)
        );
    }

...
pub fn generate_csv(report: &DedupReport, all_records: &[TenantRecord]) -> Result<Vec<u8>> {
    let mut buffer = Vec::new();
    {
        ...
        write_flagged_groups(&mut writer, &report.flagged_groups)?;
...
fn write_flagged_groups(
    ...
    for (i, flagged) in groups.iter().enumerate() {
        ...
        write_group_rows(writer, &flagged.group, &flagged.note)?;
    }
    Ok(())
}
...
fn write_typo_variant_section(
    ...
    for (i, candidate) in candidates.iter().enumerate() {
        ...
        for group in [find(&candidate.key_a), find(&candidate.key_b)].into_iter().flatten() {
            let note = if wrote_note { "" } else { candidate.note.as_str() };
            write_group_rows(writer, group, note)?;
            wrote_note = true;
        }
    }
    Ok(())

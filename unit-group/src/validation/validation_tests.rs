...
fn detects_duplicate_unit_numbers() {
    ...
    assert!(issues
        .iter()
        .any(|i| { i.description == "Duplicate unit numbers" && i.severity == Severity::Error }));
}
...
fn rare_group_is_a_warning_not_an_error() {
    ...
    let rare = issues
        .iter()
        .find(|i| i.description == "Rare UnitGroup detected")
        .expect("expected a rare-group issue");

    assert_eq!(rare.severity, Severity::Warning);
}
...
fn blank_unitgroup_is_an_error_not_a_warning() {
    let document = CsvDocument {
        ...
        file_name: "test.csv".to_string(),
        headers: vec!["number".to_string(), "unitgroup".to_string()],
        rows: vec![vec!["A01".to_string(), "".to_string()]],
    };
    ...
    let blank = issues
        .iter()
        .find(|i| i.description == "Blank UnitGroup values")
        .expect("expected a blank-UnitGroup issue");

    assert_eq!(blank.severity, Severity::Error);
}
...
fn detects_climate_mismatch() {
    ...
    let issue = issues
        .iter()
        .find(|i| i.description == "Climate status does not match UnitGroup")
        .expect("expected a climate-mismatch issue");
...
fn detects_locality_mismatch() {
    let document = CsvDocument {
        ...
        rows: vec![vec![
            "A01".to_string(),
            "10x10 Outside Non-Climate".to_string(),
            "Inside".to_string(),
        ]],
    };
    ...
    let issue = issues
        .iter()
        .find(|i| i.description == "Locality does not match UnitGroup")
        .expect("expected a locality-mismatch issue");
...
fn detects_unitgroup_dimension_mismatch() {
    let document = CsvDocument {
        ...
        rows: vec![vec![
            "A01".to_string(),
            "10x20 Inside Climate".to_string(),
            "10".to_string(),
            "15".to_string(),
        ]],
    };
    ...
    let issue = issues
        .iter()
        .find(|i| i.description == "UnitGroup dimensions do not match Width/Length")
        .expect("expected a dimension-mismatch issue");
...
fn rare_group_flags_group_names_not_unit_numbers() {
    ...
    assert!(rare.flagged_are_group_names);
...
fn invalid_dimensions_flags_unit_numbers_not_group_names() {
    ...
    assert!(!invalid.flagged_are_group_names);
    assert_eq!(invalid.flagged_values, vec!["A01".to_string()]);
}
...
fn detects_casing_mismatch() {
    let document = CsvDocument {
        ...
        headers: vec!["number".to_string(), "unitgroup".to_string()],
        rows: vec![
            vec!["K10".to_string(), "10x10 Climate".to_string()],
            vec!["k10".to_string(), "10x10 Climate".to_string()],
        ],
    };
    ...
    assert!(issues
        .iter()
        .any(|i| { i.description == "Inconsistent unit-number casing" }));
}
...
fn validate_document_errors_loudly_when_a_supposed_unit_file_has_no_matching_columns() {
    ...
    assert!(err.to_string().contains("units.csv"));
}
...
fn validates_a_unit_file_with_an_underscored_unitgroup_header() {
    let document = CsvDocument {
        modified_at: None,
        file_name: "units.csv".to_string(),
        headers: vec!["number".to_string(), "unit_group".to_string()],
        rows: vec![vec!["A01".to_string(), "".to_string()]],
    };
    ...
    assert!(issues
        .iter()
        .any(|i| { i.description == issues::BLANK_UNITGROUP }));
}

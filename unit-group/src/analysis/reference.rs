...
pub fn select_group_document<'a>(
    ...
    match &discovery.selected_group_file_name {
        Some(selected_file) => {
            tracing::info!(
                file = %selected_file,
            ...
            documents.iter().find(|d| d.file_name == *selected_file)
        }

        None => documents
            .iter()
            .find(|d| discovery.group_file_names.contains(&d.file_name)),
    }
}

pub fn load_reference_groups_from_document(document: &CsvDocument) -> Result<Vec<String>> {
    ...
    let name_index = document.header_index("name").ok_or_else(|| {
        anyhow::anyhow!(

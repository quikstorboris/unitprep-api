...
pub fn analyze_batch(
        ...
        &reference_groups
    {
        ...
        let mut occurrences: Vec<(String, String)> =
            Vec::new();

        let mut distinct_net_new: HashSet<String> =
            HashSet::new();
        ...
        for group in &distinct_net_new {
            let Some(Some((candidate, score))) =
                best_match_cache.get(group)
            else {
                continue;
            };
...
mod tests {
    ...
    fn a_group_recurring_across_facilities_yields_one_similarity_match_but_one_issue_per_facility(
        ...
        let results = analyze_batch(
            batch,
            Some(vec![reference_name.to_string()]),
        )
        .unwrap();
        ...
        assert_eq!(
            results.similar_groups[0].facility_group,
            group_name
        );
        assert_eq!(
            results.similar_groups[0].reference_group,
            reference_name
        );
    ...
    fn no_reference_groups_treats_every_global_group_as_net_new() {
        ...
        let results =
            analyze_batch(batch, None).unwrap();
